...
class _EditProfileState extends State<EditProfile> {
  ...
  void fillUserInfo(UserController userController) {
    setState(() {
      _nameController.text = userController.userLoginModel.value.userInfo![0].name!;
      _emailController.text = userController.userLoginModel.value.userInfo![0].email!;
      _mobileNoController.text = userController.userLoginModel.value.userInfo![0].phone!;
    ...
    child: SingleChildScrollView(
      child: Column(
        children: [
            ...
            child: Container(
              ...
              child: TextFormField(
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: const Icon(FontAwesomeIcons.calendarAlt, color: CColor.themeColor,),
          ...
          Padding(
            padding: EdgeInsets.symmetric(horizontal: size * .06),
            child: _customTextFormField(size, _institutionController, 'স্কুল/কলেজ/বিশ্ববিদ্যালয়'),
          ),
          SizedBox(height: size * .04,),
          Padding(
            padding: EdgeInsets.symmetric(horizontal: size * .06),
          ...
          Padding(
            padding: EdgeInsets.symmetric(horizontal: size * .06),
            child: _customTextFormField(size, _subjectController, 'বিভাগ/বিষয়'),
          ),
          SizedBox(height: size * .04,),
          Padding(
            padding: EdgeInsets.symmetric(horizontal: size * .06),
            child: _customTextFormField(size, _guardianMobileNoController, 'অভিভাবকের মোবাইল নম্বর'),
          ),
          SizedBox(height: size * .04,),
          Padding(
            padding: EdgeInsets.symmetric(horizontal: size * .06),
            child: _customTextFormField(size, _genderController, 'লিঙ্গ (পুরুষ/মহিলা)'),
          ),
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    if (picked != null && picked != selectedDate) {
      setState(() {
        selectedDate = picked;
        _birthDateController.text = enToBnNumberConvert("${selectedDate.toLocal()}".split(' ')[0]);
      });
    }
  }
}
