...
class _BoighorPublicationTabPageState extends State<BoighorPublicationTabPage> {
  ...
  Future<void> _customInit(EbookApiController ebookApiController) async {
    _count++;
    setState(() {
      _publicationsLength = ebookApiController.publicationList.length > 9
          ? 10
          : ebookApiController.publicationList.length;
      _loading = true;
    });
    await ebookApiController
        .getPublicationWiseBookList(
            ebookApiController.publicationList[0].id.toString())
        .then((value) {
      setState(() {
        _loading = false;
        _bookListLength = ebookApiController.publicationWiseBookList.length > 9
            ? 9
    ...
    // ignore: avoid_print
    print(
        'publication length = $_publicationsLength booklist length = $_bookListLength');
  }
          ...
          children: [
            Container(
                ...
                child: ListView.builder(
                    ...
                    itemBuilder: (context, index) => _publicationsLength > 9
                        ? index != _publicationsLength - 1
                            ? GestureDetector(
                                onTap: () {
                              ...
                              children: [
                                Column(
                                  ...
                                  children: [
                                    ...
                                    /// publication name
                                    Text(
                                      ebookApiController.publicationList[index]
                                          .publicationName!,
                                      style: Style.bodyTextStyle(
                                          size * .035,
                                          _tappedIndex == index
